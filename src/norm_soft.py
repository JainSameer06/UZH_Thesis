...
Usage:
  ...
  norm_soft.py test [--dynet-mem MEM] [--beam=BEAM] [--pred_path=PRED_FILE] [--input_format=INPUT_FORMAT]
    MODEL_FOLDER --test_path=TEST_FILE [--lowercase=LOW] [--conll_format]
  norm_soft.py ensemble_test [--dynet-mem MEM] [--beam=BEAM] [--pred_path=PRED_FILE] [--input_format=INPUT_FORMAT]
    ED_MODEL_FOLDER MODEL_FOLDER --test_path=TEST_FILE [--lowercase=LOW] [--conll_format]
...
def load_data(filename, input_format, lowercase=False, conll_format=False):
    """ Load data from file
...
#    def predict_next_(self, state, scores=False, hidden=False, *args, **kwargs):
...
#                log_probs_expr = dy.log(probs)
                log_probs_expr = dy.log(self.predict_next_(s))
                log_probs = log_probs_expr.npvalue()
                top = np.argsort(log_probs)[-beam:]
#                print 'expansions ' + u', '.join([self.char_vocab.i2w.get(pred_id,UNK_CHAR) for pred_id in top])
...
#                    word.append(self.char_vocab.i2w.get(pred_id,UNK_CHAR))
                    word.append(pred_id)
...
#            print u', '.join([u''.join(word) for s, log_p, word in hypos])
        ...
        # nothing found
            complete_hypotheses = [(log_p,u''.join([self.char_vocab.i2w.get(pred_id,UNK_CHAR) for pred_id in word])) for s, log_p, word in hypos]

        complete_hypotheses.sort(key=lambda h: h[0], reverse=True)
        final_hypos = []
        for log_p, word in complete_hypotheses[:beam]:
            final_hypos.append((log_p, u''.join([self.char_vocab.i2w.get(pred_id,UNK_CHAR) for pred_id in word])))
...
#        print complete_hypotheses[:beam]
    ...
    def predict(self, input, beam_size, ignore_first_eol=False, as_arrays=False):
        ...
        for i in range(MAX_PRED_SEQ_LEN):
            ...
            logprobs = np.array([-dy.log(self.predict_next_(s)).npvalue() for s in states])
#            print logprobs
...
def predict_ensemble(nmt_models, input, beam_size, ignore_first_eol=False, as_arrays=False):
    ...
    for i in range(MAX_PRED_SEQ_LEN):
        ...
        for j,m in enumerate(nmt_models):
            logprobs_m = np.array([-dy.log(m.predict_next_(s)).npvalue() for s in states[j]]) # beam_size x vocab_len matrix
        #            print logprobs
        #            print all_masks[-1, :, None]
...
#        logprobs_lst = np.array([[-dy.log(m.predict_next_(s)).npvalue() for s in m_states] for m,m_states in zip(nmt_models,states)])
#        print logprobs_lst
...
if __name__ == "__main__":
    ...
    if arguments['train']:
        ...
        train_data = data_set.from_file(train_path,input_format, arguments['--lowercase'], arguments['--conll_format'])
        print 'Train data has {} examples'.format(train_data.length)
        dev_path = check_path(arguments['--dev_path'], 'dev_path')
        dev_data = data_set.from_file(dev_path,input_format, arguments['--lowercase'], arguments['--conll_format'])
...
#                if loss is not None:
        ...
        data_set = SoftDataSet
        input_format = [int(col) for col in arguments['--input_format'].split(',')]
        test_data = data_set.from_file(test_path,input_format, arguments['--lowercase'], arguments['--conll_format'])
